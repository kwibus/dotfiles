...
module MyMouseResizableTile (
                                    -- * Usage
                                    -- $usage
                                    mouseResizableTile,
                                    mouseResizableTileMirrored,
                                    MRTMessage (ShrinkSlave, ExpandSlave,Mirror),
...
import XMonad hiding (Mirror,tile, splitVertically, splitHorizontallyBy)
...
instance LayoutClass MouseResizableTile Window where
    ...
    handleMessage st m
        | Just Mirror  <- fromMessage m =
            return $ Just st {isMirrored = not $ isMirrored st }
